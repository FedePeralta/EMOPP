...
def evalTrajectory(ind):
    ...
    reward = fleet1.move()
    distance = fleet1.ships[0].n_movements
    return tuple(list(reward) + [distance])
...
for cxpb, mutpb in parameters:
    CXPB, MUTPB, NGEN = cxpb, mutpb, ngen    
    res_individuos = open('Results/ind_multi_' + str(n_ships) + 'ships_distance.txt', 'a')
    res_fitness = open('Results/fitness_multi_' + str(n_ships) + 'ships_distance.txt', 'a')
