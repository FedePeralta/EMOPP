...
f3 = pd.read_csv(f"Results/fitness_multi_{number_of_ships}ships.txt", index_col=0)
ind = []
with open(f"Results/ind_multi_{number_of_ships}ships.txt") as f:
    line = f.readline()
    while line != "":
...
obj_space.scatter(np.zeros_like(f3["hb"]), f3["hb"], f3["rb"], c=np.zeros_like(f3["sh"]), cmap='binary_r', alpha=0.05)
obj_space.scatter(f3["sh"], np.zeros_like(f3["hb"]), f3["rb"], c=np.zeros_like(f3["sh"]), cmap='binary_r', alpha=0.05)
obj_space.scatter(f3["sh"], f3["hb"], np.zeros_like(f3["hb"]), c=np.zeros_like(f3["sh"]), cmap='binary_r', alpha=0.05)
